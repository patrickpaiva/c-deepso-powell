...
def experimentacao_powell(function, dimension, swarm_size, lower_bound, upper_bound,
                          percent_powell_start_moment, percent_powell_func_evals, wi, wa, wc, tcom, tmut, max_v,
                          max_fun_evals, max_iter, processes_per_batch=6):
    results = []
    global_best_data = []
    total_executions = 25
    completed_executions = 0

    while completed_executions < total_executions:
        batch_size = min(processes_per_batch, total_executions - completed_executions)
        
        with ProcessPoolExecutor(max_workers=batch_size) as executor:
            futures = [
                executor.submit(executar_experimento, function, dimension, swarm_size, lower_bound, upper_bound,
                                percent_powell_start_moment, percent_powell_func_evals, wi, wa, wc, tcom, tmut,
                                max_v, max_fun_evals, max_iter, i + completed_executions)
                for i in range(batch_size)
            ]
            
            with tqdm(total=batch_size, desc="Executando em paralelo...", unit="iter") as pbar:
        ...
        completed_executions += batch_size
        gc.collect()  # Coletar lixo após cada lote de processos
